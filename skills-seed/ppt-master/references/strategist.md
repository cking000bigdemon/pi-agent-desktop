...
**When selection includes B**, you must run `"$PI_BUNDLED_PYTHON" scripts/analyze_images.py <project_path>/images` before outputting the spec, and integrate scan results into the image resource list.
...
```bash
"$PI_BUNDLED_PYTHON" scripts/project_manager.py init <project_name> --format <canvas_format>
```
