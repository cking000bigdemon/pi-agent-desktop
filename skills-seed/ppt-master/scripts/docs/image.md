...
```bash
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "A modern futuristic workspace"
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "Abstract tech background" --aspect_ratio 16:9 --image_size 4K
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "Concept car" -o projects/demo/images
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "Beautiful landscape" -n "low quality, blurry, watermark"
"$PI_BUNDLED_PYTHON" scripts/image_gen.py --list-backends
```

Backends are grouped into Core / Extended / Experimental tiers. Run `"$PI_BUNDLED_PYTHON" scripts/image_gen.py --list-backends` for the current list.

Backend selection:

```bash
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "A cat" --backend openai
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "A cinematic portrait" --backend minimax
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "A product launch hero image" --backend qwen
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "科技感背景图" --backend zhipu
"$PI_BUNDLED_PYTHON" scripts/image_gen.py "A product KV in cinematic style" --backend volcengine
```
...
```bash
"$PI_BUNDLED_PYTHON" scripts/analyze_images.py <project_path>/images
```
...
```bash
"$PI_BUNDLED_PYTHON" scripts/image_search.py "offshore wind farm" \
...
```bash
# Pin Wikimedia
"$PI_BUNDLED_PYTHON" scripts/image_search.py "Olympics opening ceremony" \
  --filename event.jpg --provider wikimedia \
  --orientation landscape -o projects/demo/images

# Strict mode — refuse CC BY / CC BY-SA
"$PI_BUNDLED_PYTHON" scripts/image_search.py "abstract gradient" \
...
```bash
"$PI_BUNDLED_PYTHON" scripts/gemini_watermark_remover.py <image_path>
"$PI_BUNDLED_PYTHON" scripts/gemini_watermark_remover.py <image_path> -o output_path.png
"$PI_BUNDLED_PYTHON" scripts/gemini_watermark_remover.py <image_path> -q
```
