...
```bash
"$PI_BUNDLED_PYTHON" -m http.server --directory <svg_output_path> 8000
```
...
```bash
"$PI_BUNDLED_PYTHON" scripts/notes_to_audio.py <project_path> --voice zh-CN-XiaoxiaoNeural
"$PI_BUNDLED_PYTHON" scripts/svg_to_pptx.py <project_path> --recorded-narration audio
```
