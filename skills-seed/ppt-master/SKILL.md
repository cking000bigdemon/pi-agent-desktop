...
> [!IMPORTANT]
> ## 🧩 Bundled Runtime (Pi Agent desktop)
>
> This skill ships with the Pi Agent app and runs with **zero setup**:
> - **Interpreter**: every command below uses `$PI_BUNDLED_PYTHON` — the app's
>   bundled Python, which already has all of this skill's dependencies
>   pre-installed (python-pptx, PyMuPDF, svglib, reportlab, Pillow, numpy,
>   mammoth, openpyxl, …). **Do NOT `pip install` anything and do NOT create a
>   `.venv` for this skill** — just run the commands as written. (`$PI_BUNDLED_PYTHON`
>   is an env var set by the app; the workspace Python guard allows it.)
> - **`${SKILL_DIR}`** = this skill's directory, typically
>   `~/.pi/agent/skills/ppt-master`. Resolve it to that absolute path when running
>   commands.
> - **Image generation** defaults to **placeholder mode** when no API key is set
>   — the full deck still generates, with `ai` image rows left as placeholders
>   (offline-safe). To enable real AI image generation, copy
>   `${SKILL_DIR}/.env.example` to `~/.ppt-master/.env` and add a provider key.
> - If a dependency genuinely is missing for an optional path (e.g. `curl_cffi`,
>   `cairosvg`, `nbconvert`), create a project `.venv` and `pip install` it there
>   per the workspace guard — do not touch the bundled interpreter.
...
|---------------|---------|
| PDF file | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/pdf_to_md.py <file>` |
| DOCX / Word / Office document | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| XLSX / XLSM / Excel workbook | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/excel_to_md.py <file>` |
| CSV / TSV | Read directly as plain-text table source |
| PPTX / PowerPoint deck | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/ppt_to_md.py <file>` |
| EPUB / HTML / LaTeX / RST / other | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/doc_to_md.py <file>` |
| Web link | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` |
| WeChat / high-security site | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/source_to_md/web_to_md.py <URL>` (requires `curl_cffi`; falls back to `node web_to_md.cjs <URL>` only if that package is unavailable) |
...
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/project_manager.py init <project_name> --format <format>
```
...
| Situation | Action |
|-----------|--------|
| Has source files (PDF/MD/etc.) | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/project_manager.py import-sources <project_path> <source_files...> --move` |
...
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/analyze_images.py <project_path>/images
```
...
|---|---|---|
| `ai` | `references/image-generator.md` | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/image_gen.py ...` |
| `web` | `references/image-searcher.md` | `"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/image_search.py ...` |
...
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/svg_quality_checker.py <project_path>
```
...
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/total_md_split.py <project_path>
```

**Step 7.2** — SVG post-processing (icon embedding / image crop & embed / text flattening / rounded rect to path):
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/finalize_svg.py <project_path>
```

**Step 7.3** — Export PPTX (embeds speaker notes by default):
```bash
"$PI_BUNDLED_PYTHON" ${SKILL_DIR}/scripts/svg_to_pptx.py <project_path>
# Output:
...
- Local preview: `"$PI_BUNDLED_PYTHON" -m http.server -d <project_path>/svg_final 8000`
